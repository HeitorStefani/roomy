...
function DueDateBadge({ dueDateStr, overdue }: { dueDateStr: string; overdue: boolean }) {
  ...
  if (days === 0)
    return <span className="flex items-center gap-1 text-[10px] text-amber-400 font-medium"><CalendarDays className="w-3 h-3" />Hoje</span>
  if (days === 1)
    return <span className="flex items-center gap-1 text-[10px] text-amber-300 font-medium"><CalendarDays className="w-3 h-3" />Amanhã</span>
  return <span className="flex items-center gap-1 text-[10px] text-zinc-500"><CalendarDays className="w-3 h-3" />{fmt} · {days}d</span>
}
      ...
      {form.recurrence !== 'única' && (
        <div className="space-y-2">
          ...
          <p className="text-zinc-600 text-[10px]">
            {form.rotationMembers.length === 0 ? 'Nenhum — todos participarão' : `${form.rotationMembers.length} participante(s)`}
          </p>
        </div>
      )}
    </div>
...
function TaskCardInner({
  task, moradores, userId, onMove, onEdit,
  isDragOverlay = false, dragListeners, dragAttributes, fading,
}: {
  task: Task; moradores: Morador[]; userId: string
  onMove: (id: string, status: Status) => void
  onEdit: (task: Task) => void
  isDragOverlay?: boolean; dragListeners?: any; dragAttributes?: any
  fading?: boolean
}) {
  ...
  const isBlocked  = !isMe && nextStatus === 'doing'
  ...
  const ActionButton = nextStatus && !isDragOverlay ? (
    isBlocked ? (
      <span className="flex items-center justify-center gap-1 text-xs text-zinc-600 bg-zinc-800 border border-zinc-700 rounded-xl px-3 py-2.5 sm:py-0.5 sm:rounded-full sm:text-[10px] cursor-not-allowed w-full sm:w-auto">
        <Lock className="w-3 h-3" /> Não é sua
      </span>
    ) : (
      <button
        ...
        disabled={moving}
        className={`flex items-center justify-center gap-1.5 font-semibold transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed w-full rounded-xl px-3 py-2.5 text-sm sm:w-auto sm:rounded-full sm:px-2 sm:py-0.5 sm:text-[10px] sm:font-normal ${
          nextStatus === 'doing'
            ? 'bg-amber-500 hover:bg-amber-400 text-zinc-900 sm:bg-zinc-700 sm:hover:bg-zinc-600 sm:text-zinc-500 sm:hover:text-white'
            : 'bg-emerald-500 hover:bg-emerald-400 text-zinc-900 sm:bg-zinc-700 sm:hover:bg-zinc-600 sm:text-zinc-500 sm:hover:text-white'
        }`}
      >
        {moving ? (
          <><Loader2 className="w-4 h-4 sm:w-3 sm:h-3 animate-spin" />Salvando</>
        ) : nextStatus === 'doing' ? (
          <><Flame className="w-4 h-4 sm:hidden" />Iniciar<ChevronRight className="w-4 h-4 sm:w-3 sm:h-3" /></>
        ) : (
          <><CheckCircle2 className="w-4 h-4 sm:hidden" />Concluir<ChevronRight className="w-4 h-4 sm:w-3 sm:h-3" /></>
        )}
      </button>
    )
  ) : null
  ...
  return (
    <div
      className={`bg-zinc-800 rounded-xl border select-none overflow-hidden ${
        isDragOverlay ? 'border-yellow-500/60 shadow-2xl shadow-black/60 rotate-1 scale-105' :
        fading        ? 'border-emerald-500/40 opacity-0 scale-95' :
        task.overdue  ? 'border-red-500/40 hover:border-red-500/60' :
                        'border-zinc-700/50 hover:border-zinc-600'
      }`}
      style={{ transition: fading ? 'opacity 600ms ease, transform 600ms ease' : 'border-color 150ms' }}
    >
      <div className="p-3 space-y-2.5">
        ...
        {task.dueDate && task.status !== 'done' && <DueDateBadge dueDateStr={task.dueDate} overdue={task.overdue} />}
        ...
        <div className="flex items-center justify-between flex-wrap gap-1.5">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs text-zinc-400">{isMe ? 'Você' : morador?.name.split(' ')[0] ?? '—'}</span>
          </div>
          <div className="flex items-center gap-1.5">
            <Badge variant="outline" className={`text-[10px] px-1.5 py-0 ${priorityStyle[task.priority as Priority]}`}>
              {task.priority}
            </Badge>
            <span className="hidden sm:block">{ActionButton}</span>
          </div>
        </div>
      </div>
      {ActionButton && <div className="px-3 pb-3 sm:hidden">{ActionButton}</div>}
    </div>
  )
}

// ── Draggable wrapper ─────────────────────────────────────────────────────────
function DraggableCard({ task, moradores, userId, onMove, onEdit, fading }: {
  task: Task; moradores: Morador[]; userId: string
  onMove: (id: string, status: Status) => void
  onEdit: (task: Task) => void; fading?: boolean
}) {
  ...
  return (
    <div ref={setNodeRef} style={{ transform: CSS.Translate.toString(transform), opacity: isDragging ? 0.3 : 1, transition: isDragging ? 'none' : 'opacity 0.15s' }}>
      <TaskCardInner task={task} moradores={moradores} userId={userId} onMove={onMove} onEdit={onEdit}
        dragListeners={listeners} dragAttributes={attributes} fading={fading} />
    </div>
  )
}

// ── Droppable Column ──────────────────────────────────────────────────────────
function DroppableColumn({ col, tasks, moradores, userId, onMove, onEdit, isOver, fadingIds }: {
  col: typeof columns[number]; tasks: Task[]; moradores: Morador[]; userId: string
  onMove: (id: string, status: Status) => void
  onEdit: (task: Task) => void; isOver: boolean; fadingIds: Set<string>
}) {
    ...
    <div className={`rounded-2xl border ${col.bg} p-3 sm:p-4 space-y-3 transition-colors duration-150 ${isOver ? col.dropBg : 'bg-zinc-800/50'}`}>
      ...
      <div ref={setNodeRef} className="space-y-2.5 min-h-[80px] sm:min-h-[120px]">
        {tasks.length === 0 ? (
          ...
          tasks.map(task => (
            <DraggableCard key={task.id} task={task} moradores={moradores} userId={userId}
              onMove={onMove} onEdit={onEdit} fading={fadingIds.has(task.id)} />
          ))
        )}
      </div>
    </div>
...
export default function TarefasClient({ data }: { data: TarefasData }) {
  ...
  const handleDragOver = (e: any) => {
    const id = e.over?.id as Status | null
    setOverColumn(id && ['todo','doing','done'].includes(id) ? id : null)
  }
    ...
    <div className="min-h-screen bg-gray-800 p-1 sm:p-2">
      <div className="bg-zinc-900 min-h-screen rounded-2xl sm:rounded-3xl p-3 sm:p-4 md:p-6 space-y-4 sm:space-y-6">
        ...
        <div className="space-y-2 sm:space-y-0 sm:flex sm:items-center sm:gap-3 sm:flex-wrap">
          <span className="text-zinc-500 text-sm flex items-center gap-1.5"><ListFilter className="w-4 h-4" />Filtrar:</span>
            ...
            {['todos', ...moradores.map((m: Morador) => m.id)].map(id => {
              ...
              return (
                <button key={id} onClick={() => setFilterMorador(id)}
                  className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium transition-all shrink-0 ${filterMorador === id ? id === 'todos' ? 'bg-zinc-600 text-white' : `${m?.avatar_color} text-white` : 'bg-zinc-800 text-zinc-400 hover:text-white border border-zinc-700'}`}>
          ...
          <div className="flex gap-1.5 overflow-x-auto pb-1 sm:pb-0 scrollbar-none">
            {['todas','única','diária','2x-semana','semanal','quinzenal','mensal'].map(r => (
              <button key={r} onClick={() => setFilterRecurrence(r)}
                className={`px-3 py-1 rounded-full text-xs font-medium transition-all capitalize shrink-0 ${filterRecurrence === r ? 'bg-zinc-600 text-white' : 'bg-zinc-800 text-zinc-400 hover:text-white border border-zinc-700'}`}>
